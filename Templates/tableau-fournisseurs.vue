                    ...
                    <div style="width:100%;height:auto;text-align:center;">                              
                        <div style="display:inline-block;" id="conteneur">
                            <div class="centre">
                                <div class="titre_centre">
                                    ...
                                    <div class="d-flex justify-content-center mb-3">
                                        ...
                                        <div>
                                            <button id="import" class="btn btn-secondary form-group tooltip-wrapper" data-bs-toggle="tooltip"  :title="translate('Vous devez passer à la version PRO.')"> {{ translate('Importer') }} </button>
                                        </div>
                                        <div>
                                            <button  id="export" class="btn btn-secondary form-group tooltip-wrapper" data-bs-toggle="tooltip"  :title="translate('Vous devez passer à la version PRO.')"> {{ translate('Exporter') }} </button>
                                        </div>                   
                                    </div>
                                </div>	
                            </div>
                            ...
                            <table class="table table-hover" v-if="fournisseurs.length > 0">
                              ...
                              <tbody>
                                <tr v-for="fournisseur in currentFournisseurs" :key="fournisseur.id">
                            ...
                            <div class="pagination">
                              <button id="precedent" class="btn btn-primary" @click="changePage(currentPage - 1)" :disabled="currentPage === 1">{{ translate('Précédent') }}</button>
                              <span>{{ currentPage }} / {{ totalPages }}</span>
                              <button id="suivant" class="btn btn-primary" @click="changePage(currentPage + 1)" :disabled="currentPage === totalPages">{{ translate('Suivant') }}</button>
                            </div>
                        </div>
                    </div>								
                </div>
...
export default {
  ...
  data() {
    return {
      userLang: wp_data.locale || 'fr_FR',
      translations: window.wp_data.translations.locale_data.messages, // Accéder à la structure imbriquée
  ...
  computed: {
    ...
    totalPages() {
      return Math.ceil(this.fournisseurs.length / this.itemsPerPage);
    },
    // Récupérer les fournisseurs pour la page actuel
    currentFournisseurs() {
      const start = (this.currentPage - 1) * this.itemsPerPage;
      return this.fournisseurs.slice(start, start + this.itemsPerPage);
    }
  },
  created() {
